...
BRATS_path = os.path.join('/home/gergely/Asztal/BRATS/training')
# change here to the directory of preferred output directory')
OUTPUT_path = os.path.join(
    os.environ['HOME'], 'Dataset', 'Brats15_training_renamed')
...
def load_scans_BRATS15(pat_folder, with_seg=False):
    ...
    for nii_folder in nii_folders:
        ...
        nii_fn = [f_n for f_n in os.listdir(nii_path)
                  if f_n.endswith(('.mha')) and f_n.startswith('SMIR')]
        assert len(nii_fn) == 1
...
def save_scans_BRATS15(pat_cat, pat_folder, pat_name, VSD_id, img_data, seg_data=None):
    save_mod_names = ['Flair', 'T1', 'T1c', 'T2']
    save_seg_name = 'Label'
    assert img_data.shape[3] == 4
    ppp = os.path.basename(pat_folder)
    _OUTPUT_path = os.path.join(OUTPUT_path, pat_cat)
    __OUTPUT_path = os.path.join(_OUTPUT_path, ppp)
    ...
    for mod_i in range(len(save_mod_names)):
        ...
        save_path = os.path.join(__OUTPUT_path, save_name) 
...
def main(pat_category_list=('HGG', 'LGG'), dataset='BRATS17', crop=False):
    for pat_cat in pat_category_list:
        ...
        for pat_folder_name in os.listdir(os.path.join(BRATS_path, pat_cat)):
            ...
            if dataset == 'BRATS15':
                save_scans_BRATS15(pat_cat, pat_folder,pat_name, VSD_id, img_data, seg_data)
