...
if __name__ == '__main__':
    ...
    net_name   = ['wt', 'tc', 'en']
    net_name_c = ['WT', 'TC', 'EN']
    num_pretrain = [10000, 10000, 10000]
    for i in range(3):
        for view in ['sg', 'cr']:
            print("pretrain: ",num_pretrain[i])
            checkpoint_from = "model{0:}_sp40_soft/msnet_{1:}32_{2:}.ckpt".format(year, net_name[i], num_pretrain[i])
            checkpoint_to   = "model{0:}_sp40_soft/msnet_{1:}32{2:}_init".format(year, net_name[i], view)
